...
### Profile
Software Engineer with production experience building distributed, multi-tenant backend systems, and a track record of delivering full-stack solutions for real clients. Currently a Software Engineering Intern at **Agento**, contributing to an AI OS product — focused on caching, polling, and observability at scale. Reading Computer Science at FAST-NUCES, Karachi.

Engineering approach is grounded in architecture-first thinking: systems are designed for performance, availability, scalability, and consistency from the outset, not retrofitted for them later.

### Technical Skills
`Python` `FastAPI` `React` `PostgreSQL` `Redis` `OpenAI API` `n8n` `C++` `Azure` `Go` `Supabase` `Render` `MongoDB`
